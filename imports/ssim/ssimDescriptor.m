function [resp,drawCoords,salientCoords,uniformCoords]=ssimDescriptor(img,parms,allXCoords,allYCoords)
...
function simMap=findSimMap(xInterior,yInterior,imgPatches,parms)
  ...
  for i=1:numBins
    if(isempty(parms.binIndices{i})), simMap(i)=0;
...
function parms=makeSimMapParams(coRelWindowRadius,imgHeight,patchRad,autoVarRadius,numRadiiIntervals,numThetaIntervals ...
  ...
  radiiQuants = zeros(1,numRadiiIntervals);
